 ...
 * @return int|WP_Error
 */
function wd_ac_insert_address(array $args = [] ): WP_Error|int{
	global $wpdb;

	if ( empty( $args['name'] ) ) {
		return new \WP_Error( 'no-name', __( 'You must provide a name.', 'cofixer' ) );
	}
	...
	if ( isset( $data['id'] ) ) {
		...
		if ( ! $inserted ) {
			return new \WP_Error( 'failed-to-insert', __( 'Failed to insert data', 'cofixer' ) );
		}
 ...
 * @return array
 */
function wd_ac_get_addresses(array $args = [] ): array{
	global $wpdb;
 ...
 * @return int
 */
function wd_ac_address_count(): int{
	global $wpdb;
 ...
 * @return object
 */
function wd_ac_get_address(int $id ): object{
	global $wpdb;
 ...
 * @return int|boolean
 */
function wd_ac_delete_address(int $id ): bool|int{
	global $wpdb;
 ...
 * @return void
 */
function wd_ac_address_purge_cache(int $book_id = null ): void{
	$group = 'address';
